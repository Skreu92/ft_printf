...
char *check_x_modifiers(t_modifiers *m, va_list params, char c)
{
	...
	if(m->l)
		...
		buffer = ft_llutoa_base((unsigned short)va_arg(params, unsigned int), base);
	else if(m->hh)
		buffer = ft_llutoa_base((unsigned char)va_arg(params, unsigned int), base);
	else if(m->j)
		buffer = ft_lutoa_base(va_arg(params, uintmax_t), base);
	else if(m->z)
		buffer = ft_lutoa_base(va_arg(params, size_t), base);
	else
